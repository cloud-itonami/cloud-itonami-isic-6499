(ns vcfund.operation
  "OperationActor -- one fund operation = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor (DD-LLM) is sealed
  into a single node (:advise); its proposal is ALWAYS routed through the
  InvestmentCommitteeGovernor (:govern) and the rollout phase gate
  ...
  One graph run = one fund operation (intake -> advise -> govern -> decide
  ...
  decision to a human Investment Committee member. The approver resumes
  with `{:approval {:status :approved}}` (or :rejected).
  `:investment/commit` and `:exit/distribute` ALWAYS reach this node when
  the governor is clean -- see `vcfund.phase`."
...
(defn build
  ...
  `vcfund.store/Store`).
  opts:
    :advisor      -- a `vcfund.ddllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (ddllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; DD-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (ddllm/-advise advisor store request)]
            {:proposal p :audit [(ddllm/trace request p)]})))

      ;; InvestmentCommitteeGovernor -- independent censor (separate system than the LLM).
      (g/add-node :govern
      ...
      ;; Approval handoff -- paused by interrupt-before; a human Investment
      ;; Committee member resumes with :approval. Then route commit/hold.
